...
- 自动抓取 PDF 附件的功能在实际使用可能会因为各种各样的限制失效。 **如果条目抓取正常，只是抓不到附件，通常是无解的。**
...
Zotero Connector 浏览器扩展在网页上抓取文献时通常可以得到两类内容： **文献信息（文献条目的元数据）** ，以及相应的 **附件（如 PDF 全文、网页截图等）** 。如果你不能正确地抓取文献信息，或者抓取后的条目里文献信息缺失，那么下面的内容会对你有所帮助。
...
### 前往文献详情页抓取 <Badge text="初级" />

Zotero Connector 在「文献搜索」界面或「全文阅读」界面进行条目抓取时非常容易失败，在部分文献检索网站抓取时可能遇到文献信息不准确的问题。建议您在文献的详情页（如下）进行条目的抓取。
